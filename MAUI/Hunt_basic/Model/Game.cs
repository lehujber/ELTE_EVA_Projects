...
namespace Hunt_basic.Model
{
    public class Game
    {
        ...
        public directions? indexToMoveDirection(int toMove, int destination)
        {
            if (toMove < 0 || this.size*this.size <= toMove || destination < 0 || this.size*this.size <= destination)
            {
                return null;
            }
            int modulo = toMove % this.size;
            int direction = toMove - destination;
            if ((modulo == 0 && direction == 1) || (modulo == this.size -1 && direction == -1) )
            {
                return null;
            }
            if (direction == 1)
